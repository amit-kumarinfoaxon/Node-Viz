...
function stringOfBuildingShapeToBuildingShapeEnum(selectedShapeType){
  console.log("m i call helper "+ selectedShapeType )
...
function makeGridAxis(rotation = 0)
{
    ...
    // Use the largest as the grid size
    if (qLine.width > qLine.length)
    {
      sizeGridHelper = qLine.width;
    }
    else
    {
      sizeGridHelper = qLine.length;
    }

    gridSize = sizeGridHelper*2.3;

    gridHelper = new THREE.GridHelper(gridSize,50);
    ...
    make3Daxis(sizeGridHelper/2,gridSize/3.5)
}
