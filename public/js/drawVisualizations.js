...
function updateDimensions(theBuilding) {
  // Update the building dimensions
  // Update the building

  console.log(theBuilding)

  qLine.opacity = theBuilding.opacity

  qLine.perimeterDepth = theBuilding.perimeterDepth

  qLine.height = theBuilding.storeyHeight

  qLine.orientation = theBuilding.orientation

  qLine.storeys = theBuilding.numStoreys
  qLine.edges = null;
  spaceLayout.edges = null;

  qLine.length = theBuilding.length
  qLine.width = theBuilding.width
  qLine.thickness = theBuilding.thickness

  materialParameters = { color: 0xffffff * Math.random(), opacity: qLine.opacity, side: 2, transparent: true };
  const choices = [ drawL, drawH, drawT, drawBox ];

  const choice = choices[theBuilding.shape];
  ...
  {
    if (theBuilding.toogleSpaceLayout)
    {
      onToogleSpaceLayout()
    }
  })
...
function addAngles() {
  ...
  function drawExteriorWalls( vertices, obj) {
    ...
    for ( let i = 0; i < vertices.length - 1; i++ ) {
      ...
      // Draw the actual glass material
  ...
  function drawWindows( length, obj ) {
    ...
    for ( let i = 0; i < obj.children[ 0 ].userData.holes.length; i++ ) {
      ...
      const geometry = new THREE.PlaneBufferGeometry( lengthDelta * wwr, qLine.height  * wwr );
    ...
    //			mesh.position.set( i * lengthDelta + 0.5 * lengthDelta, qLine.overhang * 0.5, qLine.height * 0.5 + qLine.height * 0.5  * wwr );
      ...
      mesh.position.set( i * lengthDelta + 0.5 * lengthDelta, 0, qLine.height * 0.5  );
